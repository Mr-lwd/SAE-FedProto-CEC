...
class FedGen(Server):
    def __init__(self, args, times):
        ...
        if args.save_folder_name == "temp" or "temp" not in args.save_folder_name:
            save_item(
                generative_model, self.role, "generative_model", self.save_folder_name
            )
        ...
        head = load_item(
            self.clients[0].role, "model", self.clients[0].save_folder_name
        ).head
    ...
    def train_generator(self):
        generative_model = load_item(
            self.role, "generative_model", self.save_folder_name
        )
    ...
    def aggregate_parameters(self):
        ...
        for w, cid in zip(self.uploaded_weights, self.uploaded_ids):
            ...
            for server_param, client_param in zip(
                head.parameters(), client_model.parameters()
            ):
