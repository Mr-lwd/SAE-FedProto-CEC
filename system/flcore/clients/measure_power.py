...
_nodes = [('module/main', '0041', '0','1'),
          ('module/cpu', '0041', '1','1'),
          ('module/ddr', '0041', '2','1'),
          ('module/gpu', '0040', '0','0'),
          ('module/soc', '0040', '1','0'),
          ('module/wifi', '0040', '2','0'),

          ('board/main', '0042', '0','2'),
          ('board/5v0-io-sys', '0042', '1','2'),
          ('board/3v3-sys', '0042', '2','2'),
          ('board/3v3-io-sleep', '0043', '0','3'),
          ('board/1v8-io', '0043', '1','3'),
          ('board/3v3-m.2', '0043', '2','3'),
          ]
...
def powerSensorsPresent():
    """Check whether we are on the TX2 platform/whether the sensors are p
resent"""
    return os.path.isdir('/sys/bus/i2c/drivers/ina3221x/0-0041/iio:device0/')
...
def readValue(i2cAddr='0041', channel='0', valType='power', deviceID='0'):
    """Reads a single value from the sensor"""
    #fname = '/sys/bus/i2c/drivers/ina3221x/0-%s/iio:device1/in_%s%s_input' % (i2cAddr, valType, channel)
    fname = '/sys/bus/i2c/drivers/ina3221x/0-{}/iio:device{}/in_{}{}_input'.format(i2cAddr, deviceID, valType, channel) # read power
...
def getModulePower():
    ...
    return float(readValue(i2cAddr='0041', channel='0', valType='power', deviceID='1'))


def getAllValues(nodes=_nodes):
    """Returns all values (power, voltage, current) for a specific set of nodes."""
    return [[float(readValue(i2cAddr=node[1], channel=node[2], deviceID=node[3], valType=valType))
...
def printFullReport():
    ...
    resultTable = []
    for descr, i2dAddr, channel, deviceID in _nodes:
        row = []
        row.append(descr)
        for valType in _valTypes:
            row.append(readValue(i2cAddr=i2dAddr, channel=channel, valType=valType, deviceID=deviceID))
...
class PowerLogger:
    ...
    def showDataTraces(self, names=None, valType='power', showEvents=True, filename='showDataTraces.png'):
    ...
    def getAveragePower(self, nodeName='module/main', valType='power', printVals=False):
        timeVals, dataVals = self.getDataTrace(nodeName=nodeName, valType=valType)
        if printVals:
            print('powers:', dataVals)
        return np.mean(np.array(dataVals))
...
def main():
    ...
    times = 10
    for i in tqdm(range(times), desc='Running testing over all test samples repeatedly for {} times'.format(times)):
