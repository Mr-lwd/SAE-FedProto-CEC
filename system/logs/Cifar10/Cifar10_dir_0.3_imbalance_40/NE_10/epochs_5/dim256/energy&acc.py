...
# 定义文件路径
log_files = [
    "../dim64/FedTGP_fd_64_bs_10_lr_0.06_mo_0.8_lam_1_batch_256.out",
    "./FedTGP_fd_256_bs_10_lr_0.06_mo_0.8_lam_1_batch_256.out",
    "../dim512/FedTGP_fd_512_bs_10_lr_0.06_mo_0.8_lam_1_batch_256.out",
    "../dim1024/FedTGP_fd_1024_bs_10_lr_0.06_mo_0.8_lam_1_batch_256.out"
]
...
for config in time_model_configs:
    ...
    for model_type, save_path in config["models"]:
        ...
        for file in log_files:
            # 去掉 .out 后缀
            legend_label = os.path.basename(file)
            legend_label = legend_label.rsplit(".", 1)[0]
...
for file in log_files:
    ...
    plt.plot(range(1, len(regular_losses) + 1)[:201], regular_losses[:201], label=f"{legend_label}")
