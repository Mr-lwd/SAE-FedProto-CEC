...
def drawplot(iter):
    with open(f'MNIST_{iter}_model_DVFS.json', 'r') as file:
        data = json.load(file)
    ...
    # 设置标题和标签
    plt.title('Relationship between CPU Frequency and Power')
    plt.xlabel('CPU Core Frequency (GHz)')
    ...
    # 显示图形
    plt.show()
    plt.savefig(f'FittingPower_dynAndstaticPower_{iter}.png', dpi=300, bbox_inches='tight')
