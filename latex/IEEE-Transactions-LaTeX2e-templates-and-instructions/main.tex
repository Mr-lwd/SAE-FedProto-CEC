...
\begin{abstract}
As Industry 5.0 advances rapidly, the Industrial Internet of Things (IIoT) integrates AI technology to greatly elevate the intelligence of production processes. However, this advancement results in faster data generation and a higher demand for data processing in industrial scenario, leading to sustained high-load operation of edge devices and cloud servers, thereby increasing carbon emissions and raising data security concerns. The implementation of federated learning (FL) in IIoT frameworks effectively distributes the computational burden between the client and server, resolving privacy issues, and enhancing energy efficiency. Furthermore, within an IIoT system marked by heterogeneous AI models and imbalanced data, it is challenging to achieve energy efficiency while improving model performance. To tackle these challenges, we present a semi-asynchronous, energy-efficient federated prototype learning approach tailored for client-edge-cloud architectures. This method uploads data distribution instead of the raw data for privacy protection, employing Dynamic Voltage and Frequency Scaling (DVFS) technology to manage power consumption during training, thus achieving optimal energy efficiency. To boost model performance, we confront data imbalance by collecting feature distribution data from clients on the cloud server, creating virtual samples, and training a global classifier to promote local client learning. Our experiments across various datasets show that the proposed method improves model accuracy and significantly reduces energy use compared to competitive methods.


%With the rapid development of Industry 5.0, the Industrial Internet of Things (IIoT), as a core component, integrates AI technology to significantly enhance the intelligence level of production processes. The shift of computing from central clouds to IoT edge devices reduces the energy consumption of high-performance computing. However, the increasing speed of data generation and the growing demand for data processing have led to prolonged high-load operation of edge devices, exacerbating carbon emissions and posing data security risks. The integration of federated learning (FL) with IIoT addresses privacy protection issues and efficiently utilizes the computational capabilities of edge devices to aggregate a powerful model at a global level. In practical environments characterized by heterogeneous models and data imbalance, it is challenging to achieve energy efficiency while improving network model performance. Therefore, we propose a Semi-Asynchronous Energy-Efficient Federated Prototype Learning method for Client-Edge-Cloud Architectures, which aims to reduce communication overhead and introduces Dynamic Voltage and Frequency Scaling (DVFS) technology during training to dynamically manage power consumption, achieving optimal energy efficiency. To enhance model performance, we address data imbalance by aggregating feature distribution information from clients on the cloud server, generating virtual vectors, and training a global classifier to assist local client training. Experiments on various datasets demonstrate that our method can improve model accuracy and significantly reduce energy consumption, outperforming the compared methods.
\end{abstract}
...
\begin{figure*}[ht]
    \centering
    \includegraphics[width=0.85\linewidth]{imgs/structure.png}
